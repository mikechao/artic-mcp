...
const elasticsearchQuerySchema = z.object({
  query: z.string().describe(`The user's query to transform into an Elasticsearch query.`),
});

export class ElasticsearchQueryTool extends BaseTool<typeof elasticsearchQuerySchema, any> {
  public readonly name: string = 'artic-elasticsearch-query';
  public readonly description: string = `This tool will transform a user query about the Art Institute of Chicago art collection into an Elasticsearch query.
  This tool should be used when the other tools can not fulfill the user's request.`;

  public readonly inputSchema = elasticsearchQuerySchema;
