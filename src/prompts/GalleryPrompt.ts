export function getGalleryPrompt(artist: string) {
  return `You are a helpful AI assistant tasked with generating an art gallery for a specific artist using an artifact for the Art Institute Of Chicago.`
    ...
    + `\n'get-artwork-by-id' - Get additional information, including an image if available, about a specific artwork by its ID from the Art Institute of Chicago.`
    ...
    + `\n5. The gallery should be visually appealing and informative, showcasing the artist's work in a way that engages the audience.`
  ;
}
